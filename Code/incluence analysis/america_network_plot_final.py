...
fof_ind_date = pd.read_pickle("./fof_ind_date_america.pkl")
...
# == adding nodes
# G.add_node('Tencent Holdings', {'Mobile': 2018-10-16 15:16:21 , 'Big Data': 2018-10-16 15:16:21, ...})
...
# Source = G.nodes['Tencent Holdings']['Big Data']


def active(node):
    # return (np.isnat(np.datetime64(str(G.nodes[n2]['Big Data']))))
    # 'node' is like 'G.nodes[n2]['Big Data']'
...
for src in G.nodes():
    ...
    for n1, n2 in list(nx.bfs_edges(G, source=src)):
        if (active(G.nodes[n2]['Big Data'])):
            act_cnt = act_cnt + 1
            act_nodes[n2] = G.nodes[n2]['Big Data']

            if G.nodes[src]['Big Data'] < G.nodes[n2]['Big Data']:
                inf_cnt = inf_cnt + 1
                result.append(n2)
    ...
    # if(active(G.nodes[src]['Big Data'])):
    act_nodes[src] = G.nodes[src]['Big Data']
...
plt.title('Original vs. Random Activation for Big Data', fontsize=20)
...
#     if G.nodes['Tencent Holdings']['Big Data'] < G.nodes[n2]['Big Data']:
#         result.append(n2)
