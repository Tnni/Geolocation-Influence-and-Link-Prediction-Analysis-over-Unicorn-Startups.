...
fof_ind_date = pd.read_pickle("./fof_ind_date_europe.pkl")
...
# == adding nodes
# G.add_node('Tencent Holdings', {'Mobile': 2018-10-16 15:16:21 , 'Internet': 2018-10-16 15:16:21, ...})
...
# Source = G.nodes['Tencent Holdings']['Internet']


def active(node):
    # return (np.isnat(np.datetime64(str(G.nodes[n2]['Internet']))))
    # 'node' is like 'G.nodes[n2]['Internet']'
...
for src in G.nodes():
    ...
    for n1, n2 in list(nx.bfs_edges(G, source=src)):
        if (active(G.nodes[n2]['Internet'])):
            act_cnt = act_cnt + 1
            act_nodes[n2] = G.nodes[n2]['Internet']

            if G.nodes[src]['Internet'] < G.nodes[n2]['Internet']:
                inf_cnt = inf_cnt + 1
                result.append(n2)
    ...
    # if(active(G.nodes[src]['Internet'])):
    act_nodes[src] = G.nodes[src]['Internet']
...
plt.title('Original vs. Random Activation for Internet', fontsize=20)
...
#     if G.nodes['Tencent Holdings']['Internet'] < G.nodes[n2]['Internet']:
#         result.append(n2)
